  ...
  return (
    <aside className="fixed left-0 top-0 h-full w-72 max-lg:hidden flex flex-col z-40 bg-white dark:bg-slate-950 rounded-r-2xl shadow-2xl shadow-teal-950/10 pt-20">
      <div className="px-6 py-4 flex flex-col gap-1 mb-6">
        {activePatient ? (
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="font-headline text-lg font-extrabold text-teal-900 dark:text-teal-100">{activePatient.fullName}</p>
      ...
      <nav className="flex-1 space-y-1">
         <Link to={`/patient-overview/${qrToken}`}  className="flex items-center gap-3 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl px-4 py-3 mx-2 transition-all">
          <span className="material-symbols-outlined">analytics</span>
          <span className="text-sm font-inter tracking-tight">Patient Overview</span>
        </Link>
        <Link to={`/add-prescription/${qrToken}`} className="flex items-center gap-3 bg-teal-50 dark:bg-teal-900/30 text-teal-700 dark:text-teal-300 font-semibold rounded-xl px-4 py-3 mx-2 transition-all">
      ...
      <div className="p-6">
        <div className="bg-surface-container-low dark:bg-slate-900 p-4 rounded-xl">
          ...
          <button onClick={() => navigate(`/add-prescription/${qrToken}`)} className="w-full bg-primary text-on-primary py-2.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-2 shadow-lg shadow-primary/10">
