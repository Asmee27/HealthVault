    ...
    <div className="bg-surface font-body text-on-surface h-screen flex flex-col overflow-hidden">
      <header className="flex items-center justify-between px-6 py-4 bg-white border-b border-outline-variant">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/')} className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-surface-container transition-colors text-on-surface">
            <span className="material-symbols-outlined">close</span>
          </button>
          <h1 className="font-headline font-bold text-lg text-on-surface">Scan Patient ID</h1>
        </div>
        <div className="w-8 h-8 rounded-full border border-outline-variant overflow-hidden">
          <img className="w-full h-full object-cover" src="https://lh3.googleusercontent.com/aida-public/AB6AXuD8D2vvj4WdL6IDNkLP44jk3ZYeb0E1Xdq8S1w-D_QChGHQOSVRpEBZGQsoRvZR21DG6K9XFxOAyM4FmBGYU1xy9IrCBqUR9d79ZGe7UK4ufE6mdvSavKvzasOOro10I_CGuyOiOP--7hhitoa59FGicakl4ZZSegQheRXuf8JbT4vRZKx-CMtFgTInph98KHvUZJ1-RkZGn8iXDtJTijHKgmeL4ZeiPhntd14_sN3faMniUop3jsSltZFQlDtbZPEXi21kMHT_bTA" alt="Doctor profile" />
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-6">
        <div className="relative w-[280px] h-[280px] mb-12">
          <div className="absolute inset-0 bg-white shadow-sm rounded-3xl border border-outline-variant/30 overflow-hidden">
            <div className="absolute inset-0 bg-surface-container-low flex items-center justify-center">
              <span className="material-symbols-outlined text-8xl text-outline-variant/20">qr_code_scanner</span>
            </div>
        ...
        <div className="text-center space-y-2 mb-12 max-w-xs">
          <p className="text-on-surface text-xl font-bold tracking-tight">Place the ID inside the frame</p>
          <p className="text-on-surface-variant text-base">Align the QR code within the corners. Scanning happens automatically.</p>
        </div>

        <div className="w-full max-w-[240px]">
          <button onClick={() => navigate('/patient-records')} className="w-full flex items-center justify-center gap-2 px-6 py-3.5 bg-primary text-on-primary rounded-full text-sm font-semibold shadow-lg shadow-primary/20 hover:bg-primary-container hover:text-on-primary-container active:scale-95 transition-all">
      ...
      <footer className="p-8 flex justify-center">
        <div className="bg-surface-container-high px-5 py-3 rounded-full flex items-center gap-3 border border-outline-variant shadow-sm">
          ...
          <span className="text-[11px] text-on-surface-variant font-bold tracking-wider uppercase">Encrypted Clinical Scanning Mode</span>
